    ...
    private function getProductProfile($cart_products, $xmeta) {
        ...
        foreach($cart_products as $i => &$product) {
            ...
            if (!empty($xmeta['product_query'])) {
                $product_info = $this->db->query("SELECT price, manufacturer_id, location, stock_status_id, jan, ean $custom_query FROM " . DB_PREFIX . "product WHERE product_id = '" . (int)$product['product_id'] . "'")->row;
                if ($product_info) {
